...
class Brain():
    def __init__(self,input_dim,output_dim):
        ...
        dense2 = layers.Dense(32,activation = tf.nn.tanh)(tf.concat([input_layer,dropout1],axis=1))
        dropout2 = layers.Dropout(0.3)(dense2)
        dense3 = layers.Dense(32, activation=tf.nn.tanh)(dropout2)
        dropout3 = layers.Dropout(0.3)(dense3)
        dense4 = layers.Dense(64,activation = tf.nn.tanh)(tf.concat([input_layer,dropout3],axis = 1))
        dropout4 = layers.Dropout(0.3)(dense4)
        dense5 = layers.Dense(64,activation = tf.nn.tanh)(tf.concat([dropout2,dropout4],axis=1))
        dropout5 = layers.Dropout(0.3)(dense4)
        output = layers.Dense(output_dim,activation = tf.nn.tanh)(tf.concat([input_layer,dropout2,dropout3,dropout4,dropout5],axis = 1))
